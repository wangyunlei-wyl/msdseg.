...
from competitors_models.sync_batchnorm import SynchronizedBatchNorm2d
...
class SSFPN(nn.Module):
    def __init__(self, backbone,
        ...
        if backbone.lower() == "resnet18":
            encoder = resnet18(pretrained=pretrained_backbone)
        elif backbone.lower() == "resnet34":
            encoder = resnet34(pretrained=pretrained_backbone)
        elif backbone.lower() == "resnet50":
            encoder = resnet50(pretrained=pretrained_backbone)
        elif backbone.lower() == "resnet101":
            encoder = resnet101(pretrained=pretrained_backbone)
        elif backbone.lower() == "resnet152":
            encoder = resnet152(pretrained=pretrained_backbone)
        else:
...
def calculate_flops():
    ...
    model1= SSFPN("resnet18",ResNet34M=False).eval()
    # print(model1)
